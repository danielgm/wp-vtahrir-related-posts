<?php
/*
Plugin Name: Voices of Tahrir Related Posts
Plugin URI: 
Description: Customization of Microkids' related posts plugin. Allows user to select related posts and assign a type and strength to the relationship.
Author: Daniel McLaren <dan@danielmclaren.com>
Version: 0.1
Author URI: http://danielmclaren.com
...
function MRP_activate() {
	...
	if( !count( $wpdb->get_results( $query ) ) ) {
		$query = "CREATE TABLE ".$wpdb->prefix."post_relationships (
					...
					rel_strength int unsigned NOT NULL,
					PRIMARY KEY  (post1_id,post2_id,rel_type)
				)";
...
function MRP_load_includes() {	
	echo '<script type="text/javascript" src="'.get_option('siteurl').'/wp-content/plugins/vtahrir-related-posts/microkids-related-posts.js"></script>'; 
	echo '<link rel="stylesheet" type="text/css" href="'.get_option('siteurl').'/wp-content/plugins/vtahrir-related-posts/microkids-related-posts.css" />';
}

function MRP_get_options_html($options, $selectedValue=NULL) {
	$str = '';
	foreach ($options as $value => $label) {
		$str .= '<option value="'.$value.'"'.($selectedValue == $value ? ' selected="true"' : '').'>'.$label.'</option>';
	}
	return $str;
}
...
function MRP_inner_custom_box() {
	...
	$rel_types = MRP_get_relationship_types();
	if (count($rel_types) > 0) $rel_types = array_combine($rel_types, $rel_types);
	if (!isset($rel_types['default'])) $rel_types['default'] = 'default';
	...
	foreach($post_types as $post_type) {
	    ...
	    echo '<div class="MRP_relatedposts_container"' . (count($related_posts[$post_type]) <= 0 ? ' style="display:none"' : '') . '>';
	    echo '<label id="MRP_relatedposts_list_label'.$ext.'" class="MRP_relatedposts_list_label">'.__( 'Related', 'microkids-related-posts' ).':</label>';
		echo '<table id="MRP_relatedposts_list'.$ext.'" class="MRP_relatedposts_list">';
		echo '<tr><th></th><th>Type</th><th>Strength</th><th>Title</th></tr>';
		if( $post_ID ) {
			if( count($related_posts[$post_type]) > 0 ) {
				foreach( $related_posts[$post_type] as $related_post ) {
					echo MRP_get_post_html($related_post, $rel_types);
				}
			}
		}
		echo MRP_get_post_html(NULL, $rel_types, false);
		echo '</table>';
		echo '</div>';
...
function MRP_save_postdata( $post_id ) {
	...
	for ($i = 0; $i < count($_POST['MRP_related_post_ids']); $i++) {
		$related_posts[] = array(
			'id' => $_POST['MRP_related_post_ids'][$i],
			'rel_type' => $_POST['MRP_related_post_types'][$i],
			'rel_strength' => $_POST['MRP_related_post_strengths'][$i]
		);
	}
...
/**
 * MRP_get_relationship_types - Get a list of relationship types being used.
 */
 function MRP_get_relationship_types() {
	global $wpdb;
	$query = "SELECT DISTINCT(rel_type) AS rel_type FROM ".$wpdb->prefix."post_relationships";
	$results = $wpdb->get_results( $query );
	if (!$results) return array();
	
	$types = array();
	foreach ($results as $result) {
		$types[] = $result->rel_type;
	}
	return $types;
}

function MRP_get_post_html($post, $rel_types, $visible=true) {
	$post_id = $post == NULL ? '0' : $post->ID;
	$rel_type = $post == NULL ? 'default' : strval($post->rel_type);
	$rel_strength = $post == NULL ? '3' : strval($post->rel_strength);

	$post_title = $post == NULL ? 'UNTITLED' : $post->post_title;
	if( $post != NULL && $post->post_status != 'publish' ) {
		$post_title = $post_title . ' ('.$post->post_status.')';
	}			
	
	$str = '';
	
	$str .= '<tr id="related-post-'.$post_id.'"' . ($visible ? '' : ' style="display:none"') . '>';
	
	$str .= '<input type="hidden" name="MRP_related_post_ids[]" class="MRP_related_post_ids" value="'.$post_id.'" />';
	
	$str .= '<td><a class="MRP_deletebtn">X</a></td>';
	
	$str .= '<td><select class="MRP_related_post_types" name="MRP_related_post_types[]">';
	$str .= MRP_get_options_html($rel_types, $rel_type);
	$str .= '</select></td>';
	
	$str .= '<td><select class="MRP_related_post_strengths" name="MRP_related_post_strengths[]">';
	$str .= MRP_get_options_html(array(
		'1' => 'Very weak', '2' => 'Weak', '3' => 'Normal', '4' => 'Strong', '5' => 'Very strong'),
		$rel_strength);
	$str .= '</select></td>';
	
	$str .= '<td class="MPR_post_title">'.$post_title.'</td>';
